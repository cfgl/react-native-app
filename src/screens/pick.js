...
import {
  ONSIGNAL_KEY, ONSIGNAL_REST_API_KEY
} from "@env"
import {
  getCurrentWeekGame,
  saveBet,
...
class Pick extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.statusGame !== this.props.statusGame && (this.props.statusGame === "SUCCESS_BET_CREATE"
      ...
      let pushIds = this.props.players.filter(
        (a) => a.user.group === this.props.user.group._id && a.user.pushUserId && a.user.pushUserId !== this.props.user.pushUserId
      ...
      axios
        .post(`https://onesignal.com/api/v1/notifications`, {
          ...
          contents: { "en": `${gameString(this.state.newBet.game)} | ${this.state.newBet.type.value} | ${this.state.newBet.method.label}` },
          headings: { "en": `${this.props.user.username} ${action === "SUCCESS_BET_CREATE" ? "place a new bet" : " update a bet"}` },
        ...
        <ScrollView
          ...
          <View
            ...
            {this.state.types.map((item, index) => {
              ...
              if (this.save && !this.save.saved && this.save.game && this.save.game.Status && (this.save.game.Status === "Final" || this.save.game.Status === "InProgress")
                  ...
                  <View key={index}>
                    <MyGames
                      ...
                      parlay={
                        (item.value === "pick1" ||
                          item.value === "pick2" ||
                          item.value === "pick3") && this.props.myParlay &&
                        this.props.myParlay.filter((i) => i.week === this.props.currentWeek)
                          .length == 1
                      }
                      type={item.label}
                    ...
                    {item.value === "pick3" ? (
                      <TouchableOpacity
                        ...
                        <TouchableOpacity onPress={() => { }}>
                          <Text style={{ color: jaune, marginLeft: 10 }}>
                            PARLAY FREE PICKS
                          </Text>
                        </TouchableOpacity>
                        <PopoverController>
                            ...
                            <React.Fragment>
                              ...
                              <Popover
                                ...
                                <Text style={{ width: 140, fontSize: 10 }}>
                                  Group free games together to win bonus points.
                                  Must win all games in parlay
                                </Text>
                              </Popover>
                            </React.Fragment>
                          )}
                          ...
                          onValueChange={() => {
                            ...
                            if (!this.state.off) {
                              this.setState(
                                ...
                                () => {
                                  let ply = this.props.myParlay.filter(
                                    (i) => i.week === this.props.currentWeek
                                  );
                                  if (this.props.myParlay && ply.length === 1) {
                                    this.props.deleteParlay(
                                      ply[0]._id,
                                      this.props.user._id,
                                      this.props.token
                                    );
                          ...
                          {({
                            ...
                            <React.Fragment>
                              ...
                              <Popover
                                ...
                                <Text style={{ width: 140, fontSize: 10 }}>
                                  Group free games together to win bonus points.
                                  Must win all free games to win parlay
                                </Text>
                              </Popover>
                            </React.Fragment>
                          )}
...
const mapStateToProps = (state) => {
  ...
  const { weekGames, bets, currentYear, players, currentWeek, myParlay, statusGame, weekstartdate,

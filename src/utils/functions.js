...
const gameString = (game) => {
  let favorite =
    game && game.PointSpread < 0 ? "@" + game.HomeTeam : game.AwayTeam;
  let dog = game && game.PointSpread > 0 ? "@" + game.HomeTeam : game.AwayTeam;

  //console.log(JSON.stringify(game, null, 2));
  return `${favorite} (${game && game.PointSpread}) - ${dog} (${game.OverUnder}) `;
};
...
const winWeekPerfecto = (allBets) => {
  let totPerfecto = 0;
  let winGames = allBets.filter(
    (i) => i.win === true && i.betType.value !== "dog game"
  );
  const haveStack = checkDuplicate(winGames);
  totPerfecto =
    winGames.length === 5 && haveStack === false
      ? winGames.reduce((a, b) => a + b.points, 0)
      : 0;
    ...
    for (let index = 0; index < element2.games.length; index++) {
      ...
      let favorite =
        element.game.HomeTeamMoneyLine < 0
          ? element.game.HomeTeam
          : element.game.AwayTeam;
      let dog =
...
const parleyPoints = (allBets, id, myparlay) => {
  ...
  for (let index = 0; index < groupedBets.length; index++) {
    ...
    let ply =
      myparlay &&
        myparlay.filter((i) => i.week === parseInt(element2.week)).length == 1
        ? true
        : false;
    myparlay;
